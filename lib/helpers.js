...
export function getItemsNFTContractIdentifier() {
    if (process.env.NEXT_PUBLIC_FLOW_ENV === "testnet") {
        const concatenatedAddr = process.env.NEXT_PUBLIC_FLOASIS_ITEMS_TESTNET_ACCOUNT.substring(2);
        return `A.${concatenatedAddr}.FLOASISItems.NFT`
    } else {
        const concatenatedAddr = process.env.NEXT_PUBLIC_FLOASIS_ITEMS_MAINNET_ACCOUNT.substring(2);
        return `A.${concatenatedAddr}.FLOASISItems.NFT`
    }
}
